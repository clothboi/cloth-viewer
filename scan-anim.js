...
const tex = new THREE.TextureLoader().load(MADRAS);
...
rainImg.src = MADRAS;   // madras — same fabric that's scanned
...
const phoneRestX = () => (VW < VH ? 1.35 : 2.4);
phone.position.set(phoneRestX(), 0.6, 1.4);
scene.add(phone);
...
let lastCX = innerWidth / 2, lastCY = innerHeight / 2, hasCursor = false;
function aimPhoneAt(cx, cy, grab) {
  if (isTouch || scanned || !slideActive()) return;
  const hit = pointerToGroundXY(cx, cy);
  if (!hit) return;
  if (auto.on) stopAuto();                 // a real cursor takes over from the auto-demo
  if (!everDragged) { everDragged = true; screen.material.map = rt.texture; screen.material.needsUpdate = true; reticle.visible = true; }
  if (!grab) dragging = true;              // grab = position the phone only (no scan); an actual move scans
  lastInteract = performance.now();
  target.x = THREE.MathUtils.clamp(hit.x, -5, 5);
  target.z = THREE.MathUtils.clamp(hit.z, -3.5, 3.5);
}
addEventListener('pointermove', (e) => { lastCX = e.clientX; lastCY = e.clientY; hasCursor = true; aimPhoneAt(e.clientX, e.clientY, false); }, { passive: true });
...
canvas.addEventListener('pointerdown', (e) => {
  if (isTouch) return;
  if (scanned) { spinDrag = true; spinLastX = e.clientX; canvas.setPointerCapture(e.pointerId); return; }
  lastInteract = performance.now();
  if (!everDragged) { everDragged = true; screen.material.map = rt.texture; screen.material.needsUpdate = true; reticle.visible = true; }
  const hit = pointerToGround(e);
  if (hit) { dragging = true; target.x = THREE.MathUtils.clamp(hit.x, -5, 5); target.z = THREE.MathUtils.clamp(hit.z, -3.5, 3.5); }
...
addEventListener('pointerup', () => { dragging = false; dialDragging = false; spinDrag = false; });
...
const onStageResize = () => {
  ...
  const rx = phoneRestX();
  if (rx !== _prevRestX.v && !everDragged) {
    target.x = rx; phone.position.x = rx;
    _prevRestX.v = rx;
  }
  camera.aspect = VW/VH;
